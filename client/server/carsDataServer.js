...
export function getCarOfTheWeek(callback) {
  setTimeout(() => {
    let car = {}

    // get car of the week details from models list 
    for(let i=0;i<models.length;i++) {
      if(models[i].id == carOfTheWeek.modelId) {
        car = Object.assign({}, models[i], { review: carOfTheWeek.review })
        break
      }
    }
...
export function getCarMakes(callback) {
  setTimeout(() => {
    callback(makes)
  }, NETWORK_LATENCY)
}

export function getCarModelsByMakeId(callback, makeId) {
  setTimeout(() => {
    let carModels = []
...
export function getCarByModelId(callback, modelId) {
  ...
  setTimeout(() => {
    ...
    for(let i=0;i<models.length;i++) {
      if(models[i].id == modelId) {
        carDetails = Object.assign({}, models[i])
        break
      }
    }
